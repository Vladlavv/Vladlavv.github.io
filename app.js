...
document.getElementById("btn-show-all").addEventListener("click",showAllVideos);
...
function searchVideo(){
  ...
  if (word)
  {
    ...
    for (let i = 0; i < videosList.length; i++)
    {
      let result = videosList[i].description.search(word);
      if(result != -1)
      {
        flag = true;
      }
    ...
    {
      document.querySelector('#results').innerHTML = `<p2>No videos found with keyword ${word}</p2>`;
    }
  }else
  {
...
document.getElementById("btn-search").addEventListener("click",searchVideo);
